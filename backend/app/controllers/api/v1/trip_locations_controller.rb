class API::V1::TripLocationsController < ApplicationController
  ...
  def index
    tls = @trip.trip_locations.includes(:location).order(:position)
    render json: tls.as_json(
      only: %i[id trip_id location_id position visited_at],
      include: { location: { only: %i[id name latitude longitude country_id] } }
    )
  end
  ...
  def find_or_create_location!
    ...
    raise ActiveRecord::RecordInvalid.new(Location.new), "location.name and location.country_id required" if lp[:name].blank? || lp[:country_id].blank?
